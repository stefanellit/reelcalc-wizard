...
const output='outputs/spinning-reel-followup';
const reconciliation=read('research/spinning-page-followup/reconciliation.json');
const images=read('research/spinning-page-followup/images-optimized.json');
...
for(const r of ready){
  ...
  const viaDealer=/tackledirect|tackletrap/.test(image.source);
  const page={reelId:r.id,path:'/reel-pages/p/'+slug,family:slugify(reel.brand+' '+reel.model),imageUrl:image.imageUrl,imageAlt:image.imageAlt,imageOriginalUrl:image.originalUrl,imageSource:viaDealer?'Product photograph from exact-family dealer listing':'Manufacturer product image',imageSourcePage:image.source,imageMethod:viaDealer?'verified-exact-family-dealer-image':'verified-manufacturer-image',verifiedLive:false,quickAnswerNote:'Using more backing can reduce how much premium line you need.'};
