(function() {
    ...
    return `
      <div class="line-selector" data-role="${role}-selector">
        <div class="line-selector-head">
          ...
          <div class="line-selector-actions">
            ${role === "backing" ? '<button type="button" class="no-backing-button" data-action="no-backing">I don\'t want to use backing</button>' : ""}
  ...
  function mountCalculator(mount, reel, lines, affiliateData, services) {
    ...
    function practicalLineWarning(mainLine) {
      if (!/baitcast|casting/i.test(String(reel.reel_type || ""))) return "";
      var material = selector.normalizedMaterial(mainLine.type);
      var sourceRange = material === "Braid"
        ? reel.reelcalc_recommended_braid
        : reel.reelcalc_recommended_mono_fluoro;
      var values = String(sourceRange || "").match(/\d+(?:\.\d+)?/g);
      if (!values || !values.length) return "";
      var minimum = Number(values[0]);
      var maximum = Number(values[1] || values[0]);
      var strength = Number(mainLine.lb);
      if (!(strength > 0) || (strength >= minimum && strength <= maximum)) return "";
      var rangeLabel = minimum === maximum ? String(minimum) : minimum + "-" + maximum;
      var direction = strength < minimum ? "thinner" : "heavier";
      var extra = material === "Braid" && strength < minimum
        ? " Very thin braid can dig into a baitcaster spool under load."
        : "";
      return '<div class="practical-warning"><strong>Outside the usual starting range:</strong> This line is ' +
        direction + " than the practical " + rangeLabel + " lb " + material.toLowerCase() +
        " range for this reel. ReelCalc will still show the capacity math, but this is not a line recommendation." +
        extra + "</div>";
    }
    ...
    function updateSuggestedSummary() {
      var setup = state.suggestedSetup;
      var main = setup && setup.main;
      var backing = setup && setup.backing;
      var yards = setup && setup.yards > 0 ? setup.yards : defaults.mainLineYards;
      if (!main || !backing) {
    ...
    function useRecommendedSetup(userInitiated) {
      ...
      state.suggestedSetup = { main: main, backing: backing, yards: suggestedMainYards };
    ...
    shadow.addEventListener("click", function(event) {
      ...
      if (button) {
        ...
        if (action === "no-backing") {
          state.mode = "capacity";
          updateMode();
          emit("reelcalc:calculator-mode-changed", { mode: state.mode, source: "backing_button" });
          calculate("no_backing_button", true);
        }
        if (action === "material") {
