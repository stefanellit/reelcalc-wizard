import { read, write, missing, reels, normalizeSku } from './research.mjs';
import { aliases, additionalHolds, supplementReviewedRows } from './reviewed-evidence.mjs';
...
const pages = read('research/spinning-page-followup/extracted.json');
const rows = [];
const number = value => {
...
write('research/spinning-page-followup/official-rows.json', rows);
const catalog = read('research/spinning-page-followup/catalog-rows.json');
supplementReviewedRows(rows, catalog, pages);
...
for (const original of missing) {
  if(additionalHolds[original.sku]) {
    result.push({id:original.id,status:'held',reasons:[additionalHolds[original.sku]]});
    continue;
  }
  const matchedSku=aliases[`${original.brand}|${original.sku}`]||original.sku;
  if(matchedSku!==original.sku && reels.some(r=>r.id!==original.id&&r.brand===original.brand&&normalizeSku(r.sku)===normalizeSku(matchedSku))) {
    result.push({id:original.id,status:'held',reasons:['Corrected model code already belongs to another database record; resolve duplicate identity first.']});
    continue;
  }
  const matches = rows.filter(r => r.brand === original.brand && normalizeSku(r.sku) === normalizeSku(matchedSku));
  ...
  const differences = [];
  if(matchedSku!==original.sku) {
    differences.push({field:'sku',before:original.sku,after:matchedSku,reason:'Exact family and size matched to manufacturer model code; stable database ID retained.'});
    reel.sku=matchedSku;
    reel.search_text=[original.search_text,matchedSku.toLowerCase()].filter(Boolean).join(' ');
  }
  ...
  const refreshDiameterLookup=['SPSW14000XGA','SRG14000SWAXG','STLSW14000XGD'].includes(original.sku);
  if (capacityConflicts.length || refreshDiameterLookup) {
    reel.capacity_reference_type='mono';
    ...
    if(refreshDiameterLookup)differences.push({field:'estimated mono diameters',before:oldMono.map(c=>[c.lb,c.diameter_in]),after:reel.capacity_options.map(c=>[c.lb,c.diameter_in]),reason:'Use the existing shared calculator lookup, not the stale imported 16 lb value. These are estimates, not published diameters.'});
  }
  reel.data_warnings = (reel.data_warnings || []).filter(w => w !== 'Source/spec should be verified before public use.');
  if(['IX1000R','IX4000R'].includes(original.sku)) {
    reel.data_warnings=reel.data_warnings.filter(w=>w!=='Mono capacity is not published; spool volume is calculated from the verified PowerPro capacity.');
    reel.reel_type='Rear-drag spinning reel';
  }
  if(refreshDiameterLookup) {
    const volumes=reel.capacity_options.map(c=>c.spool_space);
    if(Math.max(...volumes)/Math.min(...volumes)>1.3) throw new Error('Capacity spread still unresolved: '+original.sku);
    reel.data_warnings=reel.data_warnings.filter(w=>w!=='Printed capacity entries vary more than expected with the calculator diameter lookup.');
  }
  reel.source_url = official.url;
  if(['IX1000R','IX4000R'].includes(original.sku))reel.source_url='https://fishshop.shimano.com/products/ix';
  if(reel.brand==='Daiwa'&&['Tatula Elite Spinning','Procyon MQ LT','Kage LT'].includes(reel.model)) {
    // The retired product URLs return 404; the manufacturer's current spec feed still contains these exact rows.
    reel.source_url=official.evidenceUrl;
  }
...
write('research/spinning-page-followup/reconciliation.json', result);
